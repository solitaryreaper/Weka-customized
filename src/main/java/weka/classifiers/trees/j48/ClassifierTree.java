...
public class ClassifierTree 
  ...
  /**
   * Returns a set of rules representing each decision path in the decision tree.
   * 
   * @param depth
   * @param prefix
   * @param ruleCollector
   */
  public void collectTreeAsDecisionRules(int depth, String prefix, List<String> ruleCollector)
  {
	  int i;
	  ...
	  try {
		  for(i=0; i < m_sons.length; i++) {
			  String localPrefix = m_localModel.leftSide(m_train) + m_localModel.rightSide(i, m_train);
			  if(!isStartOfRule) {
				  localPrefix = " AND " + localPrefix;
			  }

			  if(m_sons[i].m_isLeaf) {
				  String rule = prefix + localPrefix + " THEN " + m_localModel.dumpLabel(i,m_train);
				  ruleCollector.add(rule);
			  }
			  else {
				  m_sons[i].collectTreeAsDecisionRules(depth + 1, prefix + localPrefix, ruleCollector);
			  }
		  }
	  }
	  catch(Exception e) {
		  System.err.println("Generation of decision lists failed. Reason : " + e.getStackTrace());
	  }
